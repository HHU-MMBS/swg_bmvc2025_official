...
# How to launch: 
# source .venv/bin/activate && bash bash/edm2_cfg_swg.sh
####################################################################
...
net_neg_pkl="https://nvlabs-fi-cdn.nvidia.com/edm2/raw-snapshots/edm2-img512-xs-uncond/edm2-img512-xs-uncond-2147483-0.100.pkl"
g_method='cfg-swg' 
poll=0 # whether to poll gpu space or not

for g_weight in "0.3,0.3"   
do
    g_weight_name=$(echo $g_weight | tr ',' '_') # Careful!!! This is a hack to replace the comma with underscore
    outname="/IN${resolution}/${model_size}/CFG_SWG/${g_weight_name}/samples/"
    outdir="$base_out_dir$outname"
    echo "Running with weight $g_weight, g_method $g_method, seeds $seeds, g_interval $g_interval"

    params="--net_pos_pkl $net_pos_pkl --net_neg_pkl $net_neg_pkl --outdir $outdir \
